...
- [x] Faza G: CUTOVER GOTOV — LAKAT 3.0 LIVE na laktarenje.com (2026-07-19). Sekvenca: LAKAT_LOCKDOWN=1 + redeploy (users→/uskoro) → prod SQL migracija+lakat3-1 na PROD bazi mbjpvhlhqfhjsollgszi (kroz Supabase SQL editor, 'Success') → git push c75da54 → deploy Ready → REMOVE lockdown + redeploy. Verificirano: 667 auto-frendstava iz Club23 (41 profil, 186 checkina očuvano), root→/login, welcome 3.0 copy live. .env.local ostaje na DEV bazi (lakat-dev) za lokalni razvoj — prod ključevi u .env.prod.backup.
