...
export const BADGE_DEFS = [
  ...
  { key: "tenure_365", category: "attendance", label: "Godina dana", description: "365 dana u ekipi", hidden: false },
  { key: "founding_member", category: "attendance", label: "Osnivač", description: "Jedan od prvih 10 članova grupe", hidden: false },
  ...
  { key: "picka_1", category: "social", label: "Jednom pička", description: "Pička mjeseca 1×", hidden: false },
  { key: "picka_3", category: "social", label: "Serijska pička", description: "Pička mjeseca 3×", hidden: false },
  { key: "picka_5", category: "social", label: "Legenda srama", description: "Pička mjeseca 5×", hidden: false },
  // cuga
  ...
  { key: "prvi_ikad", category: "hidden", label: "Prvi ikad", description: "Prvi checkin u povijesti grupe", hidden: true },
...
function currentStreak(daySet, todayKey) {
  let current = 0;
  let cursor = daySet.has(todayKey) ? todayKey : addDay(todayKey, -1);
...
async function evalStreak({ admin, userId, groupId, earned, checkedInAt }) {
...
async function evalAttendance({ admin, userId, groupId, earned }) {
  const won = [];
  ...
  if (!earned.has("tenure_365")) {
    ...
    if (membership?.joined_at) {
      const days = (Date.now() - new Date(membership.joined_at).getTime()) / (24 * 60 * 60 * 1000);
      if (days >= 365) won.push("tenure_365");
    }
  }

  if (!earned.has("founding_member")) {
    const { data: earliest } = await admin
      ...
      .limit(10);
    if ((earliest ?? []).some((m) => m.user_id === userId)) won.push("founding_member");
  }

  return won;
}

async function evalReactions({ admin, userId, groupId, earned }) {
...
async function evalComments({ admin, userId, groupId, earned }) {
...
function evalHiddenCheckin({ userId, earned, checkedInAt, groupTotalCheckins }) {
  const won = [];
  if (!earned.has("night_owl") && checkedInAt && zagrebHour(checkedInAt) === 3) {
    won.push("night_owl");
  }
  if (!earned.has("prvi_ikad") && groupTotalCheckins === 1) {
    won.push("prvi_ikad");
  }
  return won;
}

async function evalDrinks({ admin, userId, groupId, earned, tonightCount = 0 }) {
  const won = [];
...
function evalPicka({ earned, pickaCount = 0 }) {
  return [1, 3, 5]
    .filter((tier) => pickaCount >= tier)
    .map((tier) => `picka_${tier}`)
    .filter((key) => !earned.has(key));
}
...
export async function evaluateBadges({ admin, userId, groupId, trigger, context = {} }) {
  try {
    ...
    if (trigger === "checkin") {
      const [streakWon, attendanceWon, reactionsWon] = await Promise.all([
        evalStreak({ admin, userId, groupId, earned, checkedInAt: context.checkedInAt }),
        evalAttendance({ admin, userId, groupId, earned }),
        evalReactions({ admin, userId, groupId, earned }),
      ]);
      let groupTotalCheckins = null;
      if (!earned.has("prvi_ikad")) {
        const { count } = await admin
          .from("checkins")
          .select("id", { count: "exact", head: true })
          .eq("group_id", groupId);
        groupTotalCheckins = count ?? null;
      }
      ...
      const [commentsWon, reactionsWon] = await Promise.all([
        evalComments({ admin, userId, groupId, earned }),
        evalReactions({ admin, userId, groupId, earned }),
      ]);
      wonKeys = [...commentsWon, ...reactionsWon];
    } else if (trigger === "shame_visit") {
      wonKeys = evalPicka({ earned, pickaCount: context.pickaCount });
    ...
    const { data: inserted } = await admin
      .from("user_badges")
      .upsert(
        wonKeys.map((badge_key) => ({ user_id: userId, group_id: groupId, badge_key })),
        { onConflict: "user_id,group_id,badge_key", ignoreDuplicates: true }
      )
      .select("badge_key");
