...
// Push članovima jedne grupe (osim pošiljatelja i excludeIds). Tko je u 2+
// grupa dobiva prefiks naziva grupe da zna koji ga šank zove.
export async function notifyGroup({
  ...
  const ids = members
    .map((m) => m.user_id)
    .filter((id) => id !== senderId && !excludeIds.includes(id));
