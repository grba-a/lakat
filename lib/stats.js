...
export function monthRanking({ profiles, daySets, monthKey, todayKey }) {
  ...
  for (const profile of profiles) {
    ...
    // "novi" — ne rangira se za pičku bez obzira na poziciju u mjesecu.
    // Za arhivske mjesece je irelevantno (stari članovi nisu novi).
...
// Novi član ima toliko dana poštede od učlanjenja (isNew u monthRanking) i
// treba bar toliko mogućih dana članstva u mjesecu (do kraja mjeseca) prije
// nego može ponijeti titulu pičke — tko je ušao prije dva dana ne smije
// odmah nositi krunu.
const MIN_POSSIBLE_DAYS = 7;

// Izjednačeni na dnu dijele titulu — samo među članovima sa stažem;
// novi članovi (grace period) su izuzeti dok pošteda ne istekne
export function worstOf(ranking) {
  const eligible = ranking.filter(
    (e) => !e.isNew && e.tenurePossible >= MIN_POSSIBLE_DAYS
  );
...
// All-time Hall of Fame — nula dodatnih upita: koristi već izračunati
// daySets (puna povijest, isti fetch koji arhiva srama već radi) i već
// izračunatu arhivu mjeseci (page već hoda kroz svaki prošli mjesec za
// prikaz arhive — ovdje se ta petlja samo ponovno iskorištava za brojanje,
// bez novog cron/tablice).
export function allTimeStats({ profiles, daySets, archive, todayKey }) {
  const pickaCount = new Map();
  for (const { losers } of archive) {
    for (const l of losers) {
      pickaCount.set(l.id, (pickaCount.get(l.id) ?? 0) + 1);
    }
  }
  ...
  for (const profile of profiles) {
    const daySet = daySets.get(profile.id) ?? new Set();
    const { longest } = computeStreaks(daySet, todayKey);
    streakLeaders.push({ ...profile, value: longest });
    attendanceLeaders.push({ ...profile, value: daySet.size });
  }
  streakLeaders.sort((a, b) => b.value - a.value || a.username.localeCompare(b.username));
  attendanceLeaders.sort((a, b) => b.value - a.value || a.username.localeCompare(b.username));

  const pickaLeaders = profiles
    .map((p) => ({ ...p, value: pickaCount.get(p.id) ?? 0 }))
    .filter((p) => p.value > 0)
    .sort((a, b) => b.value - a.value || a.username.localeCompare(b.username));

  return { streakLeaders, attendanceLeaders, pickaLeaders };
}
