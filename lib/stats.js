...
export function monthRanking({ profiles, daySets, monthKey, todayKey }) {
  ...
  for (const profile of profiles) {
    ...
    entries.push({ ...profile, days, possible, pct: days / possible });
  }
...
// Izjednačeni na dnu dijele titulu
export function worstOf(ranking) {
  if (!ranking.length) return [];
  const worstPct = ranking[0].pct;
  return ranking.filter((e) => e.pct === worstPct);
}
