...
export async function computeLiga({ admin, weekKey }) {
  ...
  // kadar_user_ids s fallbackom bez kolone (schema možda još nije
  // primijenjena — liga ne smije pasti zbog toga)
  let { data: checkins, error: checkinsError } = await admin
    .from("checkins")
    .select("user_id, group_id, checked_in_at, saziv_id, kadar_user_ids")
    .is("cancelled_at", null)
    .gte("checked_in_at", fromIso)
    .lt("checked_in_at", toIso);
  if (checkinsError) {
    ({ data: checkins } = await admin
      .from("checkins")
  ...
  const table = (groups ?? []).map((g) => {
    ...
    const kadarDani = new Set(); // max jedan kadar bonus po danu (anti-farm)
    for (const c of rows) {
      dani.add(`${c.user_id}|${getDayKey(c.checked_in_at)}`);
      aktivni.add(c.user_id);
      if (c.saziv_id) odazivi.add(`${c.user_id}|${c.saziv_id}`);
      if ((c.kadar_user_ids?.length ?? 0) >= 2) {
        kadarDani.add(getDayKey(c.checked_in_at));
      }
    }
    ...
    return {
      ...
      // SAMO serijalizabilna polja — check funkcija NE smije van
      // (server→client granica: page.jsx šalje izazov u <Sank>, RSC ne
      // zna serijalizirati funkcije pa cijeli Šank pukne na "Ups")
      izazov: {
        key: izazov.key,
        label: izazov.label,
