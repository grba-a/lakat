...
// activeCount, points }] sortirano po bodovima. Dolazak = jedinstveni
// (user, dan) par ×2; ispunjen odaziv = jedinstveni (user, saziv) ×1.
export async function computeLiga({ admin, weekKey }) {
  ...
  const stats = new Map(); // groupId -> { dani: Set, odazivi: Set, aktivni: Set }
  for (const c of checkins ?? []) {
    if (!stats.has(c.group_id)) {
      stats.set(c.group_id, { dani: new Set(), odazivi: new Set(), aktivni: new Set() });
    }
    const s = stats.get(c.group_id);
    s.dani.add(`${c.user_id}|${getDayKey(c.checked_in_at)}`);
    s.aktivni.add(c.user_id);
    if (c.saziv_id) s.odazivi.add(`${c.user_id}|${c.saziv_id}`);
  }
  ...
  const table = (groups ?? []).map((g) => {
    ...
    return {
      ...
      activeCount: s?.aktivni.size ?? 0,
      points: (s?.dani.size ?? 0) * BOD_DOLAZAK + (s?.odazivi.size ?? 0) * BOD_ODAZIV,
    };
  });
