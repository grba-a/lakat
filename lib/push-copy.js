...
// Saziv "Dižem ekipu" — ide cijeloj grupi; kad je null znači ODMAH
export function sazivPushBody(ime, mjesto, kad) {
  if (!kad) {
    return pick([
      `📣 ${ime} diže ekipu — ${mjesto}, ODMAH. Ostavi sve.`,
      `📣 ${ime} zove: ${mjesto}, sad. Tko ne dođe, časti.`,
      `📣 ${ime} je digao uzbunu — ${mjesto}. Kreći.`,
    ]);
  }
  return pick([
    `📣 ${ime} diže ekipu — ${mjesto} u ${kad}. Javi se.`,
    `📣 ${ime} zove: ${mjesto}, ${kad}. Stižeš ili ne?`,
    `📣 ${ime} skuplja ekipu za ${kad} — ${mjesto}. Bez izmotavanja.`,
  ]);
}
