...
// 3.0: bez groupId — RLS scopa na mene + frendove (za feed) ili se
// filtrira po userId (za profil).
export async function fetchAllCheckins(supabase, userId, sinceIso) {
  const PAGE = 1000;
  const all = [];
  for (let from = 0; ; from += PAGE) {
    ...
    // streak titule); profil/wrapped ne prosljeđuju sinceIso pa idu na sve.
