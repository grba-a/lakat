  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-sm flex-col justify-center px-5 py-10">
      ...
      <p className="mt-3 text-sm uppercase tracking-widest text-muted">
        ...
        Zaboravio si lozinku? Klasika.
      </p>

      {state?.ok ? (
        <p className="mt-12 rounded-card border border-accent/30 bg-accent/10 px-4 py-3 text-sm font-bold text-accent">
          {state.message}
        </p>
      ) : (
        <form
        ...
        >
          ...
          >
            {isPending ? "Malo strpljenja..." : "Pošalji mi link"}
          </button>
        </form>
      )}
