...
export default function UsernameForm({ username }) {
  ...
  return (
    <form action={formAction} className="surface-2 rounded-card px-4 py-4">
      ...
      {state?.ok && (
        <p className="mt-3 rounded-card border border-accent/30 bg-accent/10 px-4 py-3 text-sm font-bold text-accent">
          {state.message}
        </p>
      )}
      ...
      <button
        type="submit"
        disabled={isPending}
        className="surface-2 pressable-soft mt-3 h-14 w-full rounded-button font-display text-xl uppercase tracking-wide text-foreground disabled:opacity-50"
      >
        {isPending ? "Sekunda..." : "Spremi ime"}
      </button>
    </form>
  );
}
